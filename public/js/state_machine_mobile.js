...
class PreSpinState {
  ...
  enterFrom(oldState) {
    ...
    if (this.stateObject.isCascadeSpin) {
      // first cascade spin
    ...
    if (!this.slotControls.isAnyLineSelected()) {
      var mess = this.stateObject.guiController.showMessage(
        "",
        "Please select a any line.",
        this,
        () => {
          this.stateObject.guiController.closePopUp(mess);
        }
      );
...
class SpinState {
  ...
  enterFrom(oldState) {
    console.log(this.toString() + " - Enter From: " + oldState.toString());
    if (this.stateObject.isCascadeSpin)
      this.subState = this.cascadeSpinState; // first cascade spins
    else if (this.slotControls.hasFreeSpin())
      this.subState = this.freeSpinState;
    else if (this.slotControls.auto) this.subState = this.autoSpinState;
    else if (this.slotControls.useManualStop())
      this.subState = this.endLessSpinState;
...
class NormalSpinState {
  ...
  calcWinHandler(win) {
    if (win) this.stateMachine.changeState(this.stateObject.winState);
    else this.stateMachine.changeState(this.stateObject.looseState);
  }
}
...
class FreeSpinState {
  ...
  enterFrom(oldState) {
    ...
    this.slotControls.setControlActivity(
      false,
      this.slotControls.auto,
      this.slotControls.auto
    ); // activity, spinButtonAcivity, autoSpinButtonAcivity
...
class WinState {
  ...
  enterFrom(oldState) {
    ...
    this.slotControls.setSpinButtonText(
      this.slotControls.freeSpins > 0
        ? this.slotControls.freeSpins
        : this.slotControls.spinTextString
    );
    ...
    if (this.stateObject.isCascadeSpin)
      this.stateObject.winShow(() => {
        this.stateMachine.changeState(this.stateObject.preSpinState);
      }); // winshow -> prespin state -> cascade spin
    else
      this.stateObject.winShow(() => {
        this.stateMachine.changeState(this.stateObject.freeInputWinState);
      });
  }
...
class LoseState {
  ...
  setInputActivity() {
    if (this.slotControls.hasFreeSpin() && this.slotControls.autoPlayFreeSpins)
      this.slotControls.setControlActivity(
        false,
        false,
        false
      ...
      this.slotControls.hasFreeSpin() &&
      !this.slotControls.autoPlayFreeSpins
    )
      this.slotControls.setControlActivity(
        false,
        true,
...
class FreeInputWinState {
  ...
  enterFrom(oldState) {
    ...
    this.setInputActivity();
    this.stateObject.freeInputWinShow((needSpin) => {
      if (needSpin)
        this.stateMachine.changeState(this.stateObject.preSpinState);
    });
  }
...
class SpinButton {
  ...
  create(offsetX, offsetY, originX, originY) {
    ...
    this.button = this.scene.add
      .sprite(this.posX, this.posY, this.spriteNormal)
