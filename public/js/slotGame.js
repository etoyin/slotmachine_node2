...
window.onload = function async () {
  let user_id = localStorage.getItem('user_id');
    console.log(`This is my user_id:=> ${user_id ? JSON.parse(user_id) : null}`);

    // this is where the spin cunt is recorded!
    fetch('/game_page_count', {
        method: 'POST',
        headers: {
...
class SlotGame extends Phaser.Scene {
  ...
  preload() {
    ...
    // Loading png images and videos
    slotConfig.sprites.forEach((s) => {
      if (s.fileName != null) {
        const fileExtension = s.fileName.split(".").pop().toLowerCase();

        if (this.textures.exists(s.name)) {
          this.textures.remove(s.name); // Uncomment for game with multiple slot scenes
        }

        if (["png", "jpg", "jpeg", "webp"].includes(fileExtension)) {
          this.load.image(s.name, "png/" + s.fileName);
        } else if (["mp4", "webm"].includes(fileExtension)) {
          this.load.video(s.name, "png/" + s.fileName, "loadeddata", false); // Load the video
        }
      }
    });
    ...
    slotConfig.symbols.forEach((s) => {
      if (s.fileName != null) {
        ...
        this.load.image(s.name, "png/gui/Symbols/" + s.fileName);
      }
    });
    slotConfig.symbols.forEach((s) => {
      if (s.fileNameBlurred != null) {
        if (this.textures.exists(s.name + "Blurred"))
          this.textures.remove(s.name + "Blurred"); // uncomment for game with multiple slot scenes
        this.load.image(
          s.name + "Blurred",
          "png/SymbolsBlurred/" + s.fileNameBlurred
        );
      }
    });
    slotConfig.symbols.forEach((s) => {
      if (s.animation != null) {
        if (this.textures.exists(s.animation))
          this.textures.remove(s.animation); // uncomment for game with multiple slot scenes
        this.load.spritesheet(s.animation, "png/SymbolsSheet/" + s.animation, {
          frameWidth: slotConfig.frameWidth,
          frameHeight: slotConfig.frameHeight,
        });
      }
    });

    this.load.spritesheet("coinspin", "png/CoinSheet.png", {
      frameWidth: 132,
      frameHeight: 132,
    });
    ...
    this.load.image("10k", "png/10000.png")
    this.load.image("11k", "png/100000.png")
    this.load.image("12k", "png/1000000.png")

    // 2) loading sounds
    this.load.audio("box_click_clip", [
      "audio/box_click.ogg",
      "audio/box_click.mp3",
    ...
    this.load.audio("background_clip", ["audio/background.mp3"]);
  ...
  create() {
    ...
    const button_plus = this.add.sprite(1460, 1020, 'money-button_plus');
    button_plus.setDepth(200);

    // Make the sprite interactive
    button_plus.setInteractive();

    // Add a click event listener
    button_plus.on('pointerdown', () => {
        // Do something when the sprite is clicked
        window.location.replace('/fund_wallet')
    });
    ...
    this.useWild =
      slotConfig.useWild &&
      slotConfig.hasOwnProperty("wild") &&
      slotConfig.wild !== null;
    ...
    }); //create slot symbols dictionary
    this.spinCount = 0;
    ...
    coinSpinAnim = this.anims.create({
      // create coin spin animation for particles
      key: "spin",
    ...
    this.add
      ...
      .play(true)
      .setScale(0.36)
      .setPosition(850, 0); // Use the same name as in the config
    ...
    this.lineButtons = slotConfig.createLineButtons
      ...
      : null; // add line buttons - optional
    ...
    this.slotControls = new SlotControls(
      ...
      this.slotPlayer,
      slotConfig.lines,
      slotConfig.lineColor,
      slotConfig.lineBetMaxValue,
      slotConfig.jackpot.defaultAmount
    );
    this.winController = new WinController(
      this,
      this.slotControls.linesController,
      slotConfig.useScatter,
      slotConfig.scatter,
      slotConfig.jackpot,
      slotConfig.winShowTime
    );
    ...
    // var bwPU = this.guiController.showPopUp(slotConfig.createBigWinPUHandler); bwPU.messageText.text = '';
    // var hwPU = this.guiController.showPopUp(slotConfig.createHugeWinPUHandler); hwPU.messageText.text = '';
    // var mwPU = this.guiController.showPopUp(slotConfig.createMegaWinPUHandler); mwPU.messageText.text = '';
    ...
    this.scale.on(
      "resize",
      function (
        gameSize // https://newdocs.phaser.io/docs/3.55.1/Phaser.Scale.Events.RESIZE
      ) {
  ...
  update(
    time,
    delta // https://newdocs.phaser.io/docs/3.52.0/focus/Phaser.Scene-update
  ) {
    ...
    this.updateEvent.events.forEach((eW) => {
      if (eW != null && eW.action != null)
        eW.action.call(eW.context, time, delta);
    });

    if (this.fpsText)
      this.fpsText.text = "fps: " + Math.round(slotGame.loop.actualFps); // for debugging puposes only, can be disabled
  }
  ...
  runSlot() {
    ...
    this.reelSpin = true;
    if (this.slotControls.auto && !this.isFreeSpin)
      this.slotControls.incAutoSpinsCounter();
    ...
    sA.add((callBack) => {
      spinReels(this, this.reels, slotConfig, callBack);
    }, this);
    sA.add((callBack) => {
    ...
    sA.add((callBack) => {
      ...
      this.endWinSearchEvent.events.forEach((eW) => {
    ...
    sA.add((callBack) => {
      ...
      if (anyWin) {
        ...
        if (jpCoins > 0)
          this.jackPotWinEvent.events.forEach((eW) => {
            if (eW != null && eW.action != null)
              eW.action.call(eW.context, wjpCoins);
          });
        ...
        if (summCoins > 0)
          this.winCoinsEvent.events.forEach((eW) => {
            if (eW != null && eW.action != null)
              eW.action.call(eW.context, summCoins);
          });

        winSpins = this.winController.getWinSpins();
        console.log("winSpins: " + winSpins);
        if (this.useLineBetFreeSpinMultiplier)
          winSpins *= this.slotControls.lineBet;
        if (winSpins > 0)
          this.freeSpinWinEvent.events.forEach((eW) => {
            if (eW != null && eW.action != null)
              eW.action.call(eW.context, winSpins);
          });
      } else {
        this.isCascadeSpin = false;
      }
  ...
  *winShowC(completeCallBack) {
    ...
    while (this.miniGame !== null || !this.guiController.hasNoPopUp()) {
      console.log(
        "wait: this.miniGame and popups:  " +
          this.miniGame +
          " : " +
          this.guiController.hasNoPopUp()
      );
      yield null;
    }
    ...
    this.winController.winSymbolShowOnce(() => {
      winShowEnd = true;
    });
    ...
    let winCoins =
      this.winController.getLineWinCoins() +
      this.winController.getScatterWinCoins();
    ...
    let bigWin =
      winCoins > 0 &&
      winCoins >= slotConfig.minWin &&
      slotConfig.useBigWinCongratulation;
    if (bigWin) {
      ...
      if (slotConfig.showWinCoinsMessage)
    ...
    if (winSpins > 0) {
      ...
      if (slotConfig.showWinFreeSpinsMessage && this.isFreeSpin)
    ...
    if (
      this.winController.scatterWin != null &&
      this.winController.scatterWin.winEventString != null
    ) {
      yield* this.wait_ms(1000);
    ...
    this.slotPlayer.addLevelProgress(
      this.useLineBetProgressMultiplier
        ? this.winSpinLevelProgress * winLinesCount * this.slotControls.lineBet
        : this.winSpinLevelProgress * winLinesCount
    ); // for each win line
    while (this.miniGame !== null || !this.guiController.hasNoPopUp()) {
      yield null;
    }
    ...
    this.playFreeSpins =
      this.slotControls.autoPlayFreeSpins && this.slotControls.hasFreeSpin();
    ...
    if (this.startFreeGames) {
      ...
      if (slotConfig.showFreeGameMessage) {
        var fgPU = this.guiController.showPopUp(
          slotConfig.createFreeGamesPUHandler
        );
  ...
  freeInputWinShow(completeCallBack) {
    this.freeInputWinCorout = new Coroutiner(
      this,
      this.freeInputWinShowC(completeCallBack)
    );
  ...
  *freeInputWinShowC(completeCallBack) {
    yield* this.wait_ms(1000);
    if (this.showAgainWin) {
      // before the spin we can show the winning lines again
      let winShowEnd = false;
      this.winController.winSymbolShowOnce(() => {
        winShowEnd = true;
      });
  ...
  *loseShowC(completeCallBack) {
  ...
  showBigWinMessage(winCoins, time) {
    ...
    if (time && time > 0)
      this.timeoutMess = setTimeout(() => {
        this.guiController.closePopUp(bwPU);
        if (this.timeoutMess) clearTimeout(this.timeoutMess);
      }, time);
  }

  showWinCoinsMessage(winCoins, time) {
    var wcm = this.guiController.showPopUp(
      slotConfig.createSmallMessagePUHandler
    );
    wcm.messageText.text = "Your win: " + winCoins + " coins!";
    if (time && time > 0)
      this.timeoutMess = setTimeout(() => {
        this.guiController.closePopUp(wcm);
        if (this.timeoutMess) clearTimeout(this.timeoutMess);
      }, time);
  }

  showWinFreeSpinsMessage(winSpins, time) {
    var wMess = this.guiController.showPopUp(
      slotConfig.createFreeSpinsWinPUHandler
    );
    wMess.messageText.text = winSpins;
    if (time && time > 0)
      new SimpleTweenFloat(
        this,
        0,
        1,
        time,
  ...
  showJackpotWinMessage(winCoins, time) {
    var jpPU = this.guiController.showPopUp(
      slotConfig.createJackpotWinPUHandler
    );
    jpPU.messageText.text = winCoins;
    this.showCoins(true);
    if (time && time > 0)
      this.timeoutMessJP = setTimeout(() => {
        this.guiController.closePopUp(jpPU);
        this.showCoins(false);
        if (this.timeoutMessJP) clearTimeout(this.timeoutMessJP);
      }, time);
  }
...
function getSymboldData(_slotConfig, spriteName) {
  for (var si = 0; si < _slotConfig.symbols.length; si++) {
    if (_slotConfig.symbols[si].name === spriteName)
      return _slotConfig.symbols[si];
  }
  return null;
}
  ...
  console.log("Save spin")
      ...
      if(!(token === "null" || token === null || token === "")){
           axios.post("https://onehubplay.com:8000/api/slot-machine/track-spin",{},{ headers: { Authorization: `Bearer ${token}` } }).then(function (response) {
              ...
              // console.log("Save spin22222222222")
...
function spinReels(scene, reels, _slotConfig, completeCallback) {
  ...
  reels.forEach((r) => {
    pA.add((callBack) => {
      var rand =
        _slotConfig.reels_simulate && _slotConfig.reels_simulate[ri] >= 0
